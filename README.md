...
| 06 | `06_validate_tempest.sh`        | Clone upstream tempest, `discover-tempest-config` → tempest.conf, tox smoke run, FAILED extraction, diagnostic dumps |
...
- [vendor/pe-ibm/](vendor/pe-ibm/) — pinned snapshot of the pe-ibm files used
  by phase 02b
- [tools/refresh_zopenstack.sh](tools/refresh_zopenstack.sh) — re-vendor the
  exclude-list from
  [ubuntu-openstack/zopenstack](https://github.com/ubuntu-openstack/zopenstack)
- [tools/run_tempest_test.sh](tools/run_tempest_test.sh) — re-run a single
  tempest test (or pattern) against an already-set-up tempest tox env
- [vendor/zopenstack/](vendor/zopenstack/) — pinned snapshot of the
  exclude-list used by phase 06
...
- `tempest/` (if 06 ran) — cloned upstream tempest, generated `tempest.conf`,
  `smoke_output.txt`, `failures.txt`
- `tempest_report/` (if 06 ran) — diagnostic dumps (`catalog_list.txt`,
  `image_list.txt`, `network_list.txt`, `hypervisor_list.txt`, etc.)
...
workflow shape (`tox -e smoke --notest`, full smoke run, `grep FAILED`,
per-test re-runs) but with two substitutions appropriate to Sunbeam:

1. **`tempest.conf` is generated by upstream `python-tempestconf`**, not
   sed-rendered from a charmed-OpenStack template. `discover-tempest-config`
   talks to keystone/nova/glance/neutron through the openrc Sunbeam produces
   and writes a working `tempest.conf` regardless of deployment shape.
2. **No K8s-charm-specific bits.** zopenstack's helpers parse `juju status
   keystone` for a unit's `private-address`, which doesn't apply to Sunbeam
   (every service is a K8s charm exposed via traefik LoadBalancer). We use
   the openrc + `openstack` CLI for the per-service diagnostics instead.

We vendor only the **exclude-list** from zopenstack (under
[vendor/zopenstack/](vendor/zopenstack/)). Refresh with
`./tools/refresh_zopenstack.sh`. The two existing exclusions were authored
for traditional charmed keystone policy — re-evaluate for keystone-k8s after
first Sunbeam run.
...
that, `discover-tempest-config` will only enable plugins for the core
services it actually finds in the catalog. If `tempest.conf` shows entries
for feature-set services, a feature was enabled somewhere — check
`tempest_report/catalog_list.txt` to see what's actually deployed.
...
Defaults to the most recent run-id; pass `--run-id <id>` to target a
specific one. Requires phase 06 to have run first so the tempest tox venv is
populated.
